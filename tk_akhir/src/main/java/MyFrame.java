...
class MyFrame 
	...
	{ 
		...
		reset.setFont(new Font("Calibri", Font.BOLD, 15));
                reset.setBackground(Color.RED);
                reset.setForeground(Color.WHITE);
		reset.setSize(100, 20); 
		reset.setLocation(250, 450); 
		reset.addActionListener(this); 
		c.add(reset); 

		tout = new JTextArea(); 
		tout.setFont(new Font("Calibri", Font.PLAIN, 15)); 
		tout.setSize(300, 370); 
	...
	{ 
		if (e.getSource() == sub) { 
			if (term.isSelected()) { 
				...
				String data3 = "Alamat : " + tadd.getText(); 
				tout.setText(data + data1 + data2 + data3); 
				tout.setEditable(false); 
				res.setText("Registrasi Berhasil!"); 
			} 
			else { 
				tout.setText(""); 
				resadd.setText(""); 
